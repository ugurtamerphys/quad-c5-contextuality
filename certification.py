...
def alpha_clique(G):
    """Method A: alpha(G) = omega(complement(G)), exact algorithm."""
    clique, _ = nx.clique.max_weight_clique(nx.complement(G), weight=None)
    return len(clique)

def alpha_brute(G):
    """Method B: scan all subsets, find maximum independent set."""
    nodes = list(G.nodes())
    n = len(nodes)
    best = 0
    for size in range(n, 0, -1):
        ...
        for subset in combinations(nodes, size):
            ...
            if subG.number_of_edges() == 0:  # independent set
                best = size
                break
        if best == size:
            break
...
print(f'Total: {len(all_graphs)}, Connected: {len(connected_graphs)}')

# Compute alpha with both methods for all connected graphs and cross-validate
mismatches = []
...
if mismatches:
    print(f'WARNING: {len(mismatches)} graphs have mismatches!')
    for G, ac, ab in mismatches:
        print(f'  clique={ac}, brute={ab}, kenarlar={list(G.edges())}')
else:
    print(f'✓ All {len(connected_graphs)} graphs: both methods agree on α.')
    print('  Alpha computation certified.')

# ## Step 2 — Lovász Theta via SCS and CLARABEL with Primal–Dual Residuals

def lovasz_theta_with_residuals(G, solver):
    """
    Compute θ(G) with primal/dual residuals.
    Returns: (theta, primal_residual, dual_residual, status)
    """
    n = G.number_of_nodes()
...
# Bulk SCS scan over all connected graphs to identify top-50
print('Computing θ for all connected n=8 graphs with SCS...')
results_all = []

for G in tqdm(connected_graphs, desc='SCS scan'):
    a = alpha_clique(G)
...
print(f'\nScan complete. Top-50 identified.')
print(f'Rank 1: graph6={top50[0]["graph6"]}, Δ={top50[0]["delta_scs"]}')

# Compute θ for top-50 with CLARABEL
print('Running CLARABEL on top-50...')

# graph6 → Graph mapping
g6_to_graph = {}
...
# Quad-C5 (Rank 1), Rank-2, Wagner graph6 encodings
...
G_r2 = g6_to_graph[top50[1]['graph6']]  # Rank 2
...
# Run each graph 5 times with each solver, record min/max interval
N_RUNS = 5
print(f'Running each graph {N_RUNS} times with both solvers...\n')
...
print(f'Quad-C5 lower bound: {lo_qc:.8f}')
print(f'Rank-2  upper bound: {hi_r2:.8f}  →  gap = {gap_r2:.2e}')
print(f'Wagner  upper bound: {hi_wg:.8f}  →  gap = {gap_wg:.2e}')
print()
...
if gap_r2 > 0 and gap_wg > 0:
    ...
    print('✗ Intervals overlap — higher precision required.')
...
print('✓ data/all_n8_results.csv saved')

# Top-50 (both solvers + residuals)
...
print('✓ data/top50_certification.csv saved')
...
print('✓ data/certified_intervals.csv saved')
print()
print('All certification data saved to data/.')

# Dual residual: duality gap = |primal_obj - dual_obj|
# Dual objective = lambda (dual variable for tr(X)=1)
# Strong duality => gap ~ 0 at optimality

def lovasz_theta_full(G, solver):
    """Compute θ, primal residual, and dual residual (duality gap)."""
    n = G.number_of_nodes()
    nodes = sorted(G.nodes())
    idx = {v: i for i, v in enumerate(nodes)}
    X = cp.Variable((n, n), PSD=True)
...
# Primal + dual residuals for top-50 with both solvers
print('Computing primal + dual residuals for top-50...')
print()
...
print()
print('✓ Dual residual computed for all top-50 graphs.')

# Update CSV with dual residuals
import csv
with open('data/top50_certification.csv', 'w', newline='') as f:
    fields = ['rank','graph6','alpha','theta_scs','theta_clarabel',
...
print('✓ data/top50_certification.csv updated with dual residuals.')
...
def eta3_objective(params):
    """Negative objective: Σᵢ (ψ·vᵢ)² with internal normalization."""
...
print(f'SLSQP optimization for η₃(Quad-C5) ({N_RESTARTS} restarts)...')
...
print(f'Feasible solutions: {len(all_vals)}/{N_RESTARTS}')
print(f'Best η₃              : {best_val:.12f}')
print(f'1 + √5               : {target:.12f}')
print(f'Difference           : {abs(best_val - target):.2e}')
...
print('=== η₃(Quad-C5) Certification Result ===')
print()
print(f'Observed interval   : [{lo_eta3:.10f}, {hi_eta3:.10f}]')
print(f'Interval width      : {interval_w:.2e}')
print(f'1 + √5              : {target:.10f}')
print(f'Target in interval? : {lo_eta3 <= target <= hi_eta3}')
print()

roots = np.roots([1, -2, -4])
print(f'Roots of x²-2x-4=0 : {sorted(roots)[::-1]}')
print(f'Positive root (1+√5): {max(roots):.10f}')
print(f'Negative root (1-√5): {min(roots):.10f}  <- outside interval')
print()

poly_check = best_val**2 - 2*best_val - 4
print(f'Polinom x²-2x-4 @ best_val : {poly_check:.2e}  (should be ≈0)')
print()

if abs(best_val - target) < 1e-8:
    print('✓ η₃(Quad-C5) = 1+√5 numerically certified.')
    print(f'  |best - (1+√5)| = {abs(best_val - target):.2e}')
    print('  x²-2x-4=0 has a unique positive root in the interval -> algebraic identity confirmed.')
else:
...
print()
print('✓ data/eta3_certification.csv saved.')
