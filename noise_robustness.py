...
def critical_visibility(alpha, eta_d, n, d):
    """Critical visibility under depolarizing noise model."""
    noise_val = n / d
    if eta_d <= alpha:
        return float('inf')  # no quantum advantage
    return (alpha - noise_val) / (eta_d - noise_val)

def noise_tolerance(v_star):
    """Maximum tolerable noise = 1 - v*"""
    return 1 - v_star

# Graphs and parameter values
graphs = [
...
print(f'{"Graph":<22} {"n":<4} {"α":<4} {"η_d":<8} {"d":<4} {"v*":<8} {"1-v* (noise tol.)":<22} {"Note"}')
print('-' * 95)
...
# Quad-C5 vs Wagner comparison
print('=== Quad-C5 vs Wagner (d=4) ===')
vs_qc = critical_visibility(3, 3.46784, 8, 4)
vs_wg = critical_visibility(3, 3.41421, 8, 4)
print(f'Quad-C5: v* = {vs_qc:.6f}  →  noise tolerance = {1-vs_qc:.6f}')
print(f'Wagner:  v* = {vs_wg:.6f}  →  noise tolerance = {1-vs_wg:.6f}')
print(f'Quad-C5 requires {(vs_wg-vs_qc)*100:.2f}% less visibility than Wagner')
print()
...
print(f'Quad-C5 (d=3): v* = {vs_qc3:.6f}  →  noise tolerance = {1-vs_qc3:.6f}')
print(f'KCBS    (d=3): v* = {vs_kcbs:.6f}  →  noise tolerance = {1-vs_kcbs:.6f}')
print()
...
ax.axhline(3, color='black', lw=1.5, ls=':', label='Classical bound α=3 (n=8)')
ax.axhline(2, color='gray',  lw=1.5, ls=':', label='Classical bound α=2 (KCBS)')

ax.set_xlabel('Visibility v', fontsize=13)
ax.set_ylabel('S(v) = v·η_d + (1-v)·n/d', fontsize=13)
ax.set_title('Quantum Value under Depolarizing Noise', fontsize=13)
